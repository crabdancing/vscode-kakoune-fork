...
let handleMaybeExitInsertMode = (input: Vscode.textCommandArgs) =>
  ...
  | "k" => toNormalMode()
  | _ =>
    ({text: "j"}: Vscode.textCommandArgs) |> Vscode.Commands.executeCommandWithArg("default:type")
...
let handleExtendSelectionsToCharacter = (
  ...
  |> Selections.extendSelectionFromActiveToCharacter(
    ...
    ~character=String.get(input.text, 0),
  )

  toNormalMode()
}

let handleSelectToCharacter = (editor: Vscode.TextEditor.t, input: Vscode.textCommandArgs) => {
  editor
  |> Vscode.TextEditor.getSelections
...
let handleSelectOnCharacter = (editor: Vscode.TextEditor.t, input: Vscode.textCommandArgs) => {
  editor
  |> Vscode.TextEditor.getSelections
...
let handleNormalMode = (editor: Vscode.TextEditor.t, input: Vscode.textCommandArgs) =>
  switch input.text {
  // Movements.
  ...
  | ";" =>
    editor
    |> Vscode.TextEditor.getSelections
    |> Selections.onlyActiveCursor(~setSelections=Vscode.TextEditor.setSelections(editor))
    |> ignore
  | " " =>
    editor
    |> Vscode.TextEditor.getSelections
    |> Selections.onlyPrimarySelection(~setSelections=Vscode.TextEditor.setSelections(editor))
    |> ignore
  ...
  | "s" => editor |> Vscode.TextEditor.getSelections |> Search.searchSelections(~editor)
  | _ => ()
  }

let onType = (args: Vscode.textCommandArgs) => {
  Vscode.Window.activeTextEditor() |> Belt.Option.map(_, e =>
    switch Mode.getMode() {
    ...
    | ExtendSelectionsToCharacter => args |> handleExtendSelectionsToCharacter(e)
    | SelectOnCharacter => args |> handleSelectOnCharacter(e)
    | ExtendSelectionsOnCharacter => args |> handleExtendSelectionsOnCharacter(e)
    }
  )
}

let activate = context => {
  context |> Vscode.overrideCommand("type", args =>
    Vscode.TextEditor.document()
    |> Belt.Option.map(_, (d: Vscode.TextDocument.t) =>
  ...
  Vscode.Commands.registerCommand("vscode-kakoune.scrollDown", Vscode.Commands.scrollHalfPageDown)
  Vscode.Commands.registerCommand("vscode-kakoune.scrollUp", Vscode.Commands.scrollHalfPageUp)
  Vscode.Commands.registerCommand("vscode-kakoune.toNormalMode", () => toNormalMode())
  Vscode.Commands.registerCommand("vscode-kakoune.toInsertMode", () => toInsertMode())
